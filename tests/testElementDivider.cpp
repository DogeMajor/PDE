...
TEST_CASE("Test ElementDivider with Point template -based vertices") {
	...
	SECTION("Generating map of midvertices should succeed") {//OK!
		...
		map<array<int, 2>, Vertex<2,Point<2, double> >* > m_p_vertex_map = divider.get_mid_vertices_map(element, commons);
		...
		map<array<int, 2>, Vertex<2, Point<2, double> >* > m_p_vertex_map2 = divider.get_mid_vertices_map(el2, commons);
